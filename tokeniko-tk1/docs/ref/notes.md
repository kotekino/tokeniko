...
- **1a — Structured relations → atomic triples (no NL). ✅ DONE.** `tools/relations.py` harvested
...
- **1b — Glosses → atomic property facts. ✅ NOUNS DONE / verbs+adj deferred.** `tools/glosses.py`
...
- **The dictionary is an explicit, interpretable semantic** (not a black-box embedding): `TKBaseDoc`
  = 2925 Oxford-3000 axes (`tools/base.py`); `TKDictionaryDoc` = Moby Word List projected **per
  sense** through WordNet against those axes (`tools/dictionary.py`, ~197k sense-vectors). All
