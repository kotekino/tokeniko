...
**Reproduce.** `tools/fragility_batch.py` (the categorized probe matrix + injector, `prepare=0` = raw
neuro-symbolic core, no Ollama pre-filter) → `tools/trace_fragility.py` (category-aware retrospective
...
Full structured account via **`tools/soak_report.py <brain_log>`** (the new soak analyzer: performance
...
(`tools/playground_bots.py`, John + Hellen). Four ladder specimens — one scalar, four manners:
...
one true coverage gap (curated, `tools/curate_add_senses.py`). See `landed.md`. NEW LEAD en
