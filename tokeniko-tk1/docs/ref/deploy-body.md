...
> `body/tools/`, with the exact install/rehearsal commands in `body/tools/README.md`. Nothing is
> installed or running: `cp`ing the plists, the `sudo` for the rotation rule and the rehearsal
> itself are the Captain's hand.
>
> | | file |
> |---|---|
> | the wrapper | `body/tools/run_service.sh` |
> | the agents | `body/tools/online.tokeniko.{api,brain,senses}.plist` (labels `online.tokeniko.*`) |
> | log rotation | `body/tools/newsyslog-tokeniko.conf` → `/etc/newsyslog.d/tokeniko.conf` |
> | the deploy loop | `body/tools/deploy.sh` (`--dry-run` reads it without touching the body) |
> | §3.5, scripted | `body/tools/verify_transplant.py` + `body/tools/baseline-macbook-2026-08-06.json` |
> | the commands | `body/tools/README.md` |

1. **A wait-for-Mongo wrapper** (`body/tools/run_service.sh`): sources `.env`, polls Mongo until it
...
2. **Three LaunchAgent plists** (`body/tools/*.plist` → installed to `~/Library/LaunchAgents/`):
...
3. **The deploy script** (`body/tools/deploy.sh`) — see §5. ✅ built.
4. ✅ **Rehearse on the MacBook**: install the plists here first (pointing at the local Mongo) and
   ...
   sequence is written out step by step in `body/tools/README.md`.
...
4. **The runtime**: Homebrew, `python@3.14` (match the MacBook's MINOR version — a differing patch
   ...
   silent. Install `body/tools/requirements-lock-<date>.txt` first, then `pip install -e .`
...
5. ✅ **Verify before deleting anything** — **scripted**: `body/tools/verify_transplant.py`
...
```
body/tools/deploy.sh                     # from the MacBook, once main is pushed
body/tools/deploy.sh --when-quiet        # …and don't interrupt him mid-sentence
body/tools/deploy.sh --dry-run           # read the whole thing without touching the body
```
...
6. **model preflight** (`body/tools/preflight_models.py`) — does `en_core_web_lg` still match the
