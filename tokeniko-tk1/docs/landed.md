...
- **Fragility map** — a 54-probe categorized battery (`tools/fragility_batch.py` + `trace_fragility.py`, `prepare=0` raw core) → root cause: geometry leaking into the truth verdict. Full observed→diagnosis→action log in `doc/ref/test-feedback.md`.
...
- **9 first-person property facts** seeded as trusted axioms — self-authored (`tools/seed_self.py`; AxiomService compiles talker=tokeniko ⇒ "I" = its own uid). "do you think / learn / perceive / value-logic …?" → **YES**, grounded in its own words.
...
- **Generic-taxonomy axiom edges, IN-MEMORY (universal-extractor v0)** — `kb_extract.extract_generic_isa_edges` (single source of truth with `tools/probe_generic_taxonomy.py`): a GENERIC/INDEFINITE bare copular noun-noun class leaf (no identity uid, not negated, not a question) → a candidate is_a edge through the SAME `gate_edge` as the definition tier. Mined at KB load inside `_load_active_kb` and unioned into the relations reader + `edge_source` — **never persisted**: archiving the source axiom retracts the edge on the next load (finding-#4 revocation durability by construction). Negated copular generics counted as future DISJOINTNESS candidates, never silently dropped. End-to-end verified: a synthetic "a thinker is a mind" axiom → edge → the real chainer derives the mind-rules for thinkers, each carrying the edge as a revocable `subj|is_a|obj` premise at trust 0.9.
...
- **Wired at every retraction door:** `AxiomService` patch(archived=True)/replace(archived)/delete, `TheoremService` same three, and `tools/revoke_edge.py` (now transitive, reusing the primitive; its flat one-level scan replaced). Delete paths cascade BEFORE the delete (descendants fall before their ground).
...
- **Exact ground truth via gloss-inversion (the survey's unlock).** Every definition was framed by
  ...
  `tools/probe_subject_wsd.py` (read-only census over all 3233): **189/2025 noun-frame subjects
...
- **The pin (`tools/pin_definition_senses.py`)** — operator-gated writer (dry-run default): patches
...
- **LIVE-VALIDATED same hour** (`#english`, playbot puppets John+Hellen via
  `tools/playground_bots.py`): all four ladder rungs — ambient question answered («yes», 0.54) ·
...
- **Imprinting + unification A applied** (`tools/seed_trust_imprint.py --apply`, verified):
...
- **bit → the one true coverage gap**: the ingestion's max_per_pos=3 cut bit.n.06 (the information
  ...
  `tools/curate_add_senses.py` — the vector computed by the SAME algorithm as the ingestion
...
- **B — the curated `preferred` flag (data, KB-homed):** WordNet's frequency order contradicts
  ...
  `tools/curate_prefer_senses.py` (idempotent, --apply gated); the author's batch: squid.n.02 ·
...
- **The seed** (`tools/seed_scaffolds.py`, --apply gated, idempotent by (category, template)):
...
- **The core** (`lib/core/untangle.py`, parser-free): saturate everything through the mirror
  ...
  never writes it.
- **The CLI** (`tools/untangle.py`): dry-run default (full report incl. cascade previews,
...
- The render: `senses/blog.py::compose_draft` grew the `digest` branch (the REAL post-material
  ...
  ship byte-identically unseeded; `tools/seed_scaffolds_digest.py` surfaced, `--apply` = the
...
- The adaptive rhythm: `reply_tempo` EMA per room (O(1) per turn, outlier-capped so an
  ...
  (`tools/seed_behavior_rules.py`, `--apply` is the Captain's hand). +15 tests
...
- **GO-LIVE, the same day** (the two steps the roadmap was still holding, both now done): the frozen
  notice is POSTED in `#privacy` (`tools/post_consent_notice.py`, dry-run by default — it opens its
...
- **`deploy.sh` hardened alongside**: install from the newest `requirements-lock-*.txt` then
  ...
  `body/tools/preflight_models.py` catches a spaCy↔`en_core_web_lg` mismatch *before* the restart
...
- **26 tk1 scripts repaired** — the `scripts/` split left every one computing `../tokeniko/.env` from
  `tools/`. A MOVE refactor is silent on anything that derives a path from `__file__`: imports
