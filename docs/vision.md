...
> **The north star of the project: *why* tokeniko exists.** When a design decision is unclear, this
> document is the tie-breaker — and it is read BEFORE the fact, not consulted when a question looks
> big enough, so that nothing is discovered late to be something the vision already settled.
>
> For *what* is here and *how* we work: `../README.md` and `../CLAUDE.md`. For *how the code works*:
> `../tokeniko-tk1/CLAUDE.md` (v1, live and frozen) and `../tokeniko-tk2/CLAUDE.md` (v2, in build).
>
> *Moved to the repository root 2026-09-17, from `tokeniko-tk1/VISION.md`, where it was buried inside
> one generation of an idea that outlives both.*
...
> **The status column below was written before go-live and has not been re-adjudicated** — several
> of these moved a long way in July 2026 (the reasoning core, the brain loop, the voice and the sleep
> cycle are live). The pillars themselves are the vision and do not move; **the live status is in the
> generations' own documents** — `../tokeniko-tk1/docs/` for v1, `../tokeniko-tk2/docs/` for v2 — and
> the «where it lives» paths are v1's. *Flagged rather than rewritten: re-scoring ten pillars is the
> author's call, not a tidy-up.*
...
- **This file** — the north star (why). It outranks everything below it.
- **`../README.md`** · **`../CLAUDE.md`** — what is in the repository, and how we work across it.
- **`../tokeniko-tk2/docs/`** — the blueprint and the live plan: `plan.md` · `roadmap.md` ·
  `landed.md`, plus the chapter that decides each component.
- **`../tokeniko-tk1/`** — v1: the running engine, its architecture, and its own status docs.
- **`notes.xlsx`**, beside this file — the author's original notebook, where the idea started.
