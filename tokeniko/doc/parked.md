...
**Multilingual scaffolds — learning a voice in the speaker's own language (2026-07-26, the author's idea + a latent coupling)** — the convergence of learned scaffolds (the accommodation, 2026-07-24) and multilingual (§1 step 2): rather than composing English and translating at the carrier, tokeniko could learn and speak NATIVE scaffolds in the exchange's language — picked up directly from what that person actually says, in their words. The register would be theirs, not a translation of his. **The latent COUPLING (live the moment multilingual lands — check before promoting anything else here):** `brain/mimicry.py` mints `template = item.original` VERBATIM, and with inbound translation `original` is the SOURCE language while the zip is English — so Lane B's whole-zip match can fire on an English zip and mint an ITALIAN template, scoped to that speaker, with no language recorded on the row. When picked, that row's text then meets the English rag2-out polisher, the English zip-verifier, and the EN→target translator: it likely survives by luck (polish rejected, verify rejected, translation a near no-op) rather than by design. **The honest minimum** whenever this is touched: record the row's language (`MEMScaffold.lang`, from the item's `source_lang`), gate the shelf by the exchange's language, and skip the carrier's translate/polish for a row already in the target language. **The fuller prize:** per-language shelves, consolidated in sleep like any mimic, so his Italian voice grows from Italian conversation. Promote when multilingual has lived a while and the coupling is either felt or deliberately claimed.
