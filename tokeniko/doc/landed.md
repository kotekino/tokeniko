...
- **Fragility map** — a 54-probe categorized battery (`scripts/tk1/fragility_batch.py` + `trace_fragility.py`, `prepare=0` raw core) → root cause: geometry leaking into the truth verdict. Full observed→diagnosis→action log in `doc/ref/test-feedback.md`.
...
- **9 first-person property facts** seeded as trusted axioms — self-authored (`scripts/tk1/seed_self.py`; AxiomService compiles talker=tokeniko ⇒ "I" = its own uid). "do you think / learn / perceive / value-logic …?" → **YES**, grounded in its own words.
...
- **Wired at every retraction door:** `AxiomService` patch(archived=True)/replace(archived)/delete, `TheoremService` same three, and `scripts/tk1/revoke_edge.py` (now transitive, reusing the primitive; its flat one-level scan replaced). Delete paths cascade BEFORE the delete (descendants fall before their ground).
...
- **Exact ground truth via gloss-inversion (the survey's unlock).** Every definition was framed by
  ...
  `scripts/tk1/probe_subject_wsd.py` (read-only census over all 3233): **189/2025 noun-frame subjects
...
- **The pin (`scripts/tk1/pin_definition_senses.py`)** — operator-gated writer (dry-run default): patches
...
- **LIVE-VALIDATED same hour** (`#english`, playbot puppets John+Hellen via
  `scripts/tk1/playground_bots.py`): all four ladder rungs — ambient question answered («yes», 0.54) ·
...
- **Imprinting + unification A applied** (`scripts/tk1/seed_trust_imprint.py --apply`, verified):
...
- **bit → the one true coverage gap**: the ingestion's max_per_pos=3 cut bit.n.06 (the information
  ...
  `scripts/tk1/curate_add_senses.py` — the vector computed by the SAME algorithm as the ingestion
...
- **B — the curated `preferred` flag (data, KB-homed):** WordNet's frequency order contradicts
  ...
  `scripts/tk1/curate_prefer_senses.py` (idempotent, --apply gated); the author's batch: squid.n.02 ·
...
- **The seed** (`scripts/tk1/seed_scaffolds.py`, --apply gated, idempotent by (category, template)):
...
- **The core** (`lib/core/untangle.py`, parser-free): saturate everything through the mirror
  ...
  never writes it.
- **The CLI** (`scripts/tk1/untangle.py`): dry-run default (full report incl. cascade previews,
...
- The render: `senses/blog.py::compose_draft` grew the `digest` branch (the REAL post-material
  ...
  ship byte-identically unseeded; `scripts/tk1/seed_scaffolds_digest.py` surfaced, `--apply` = the
...
- The adaptive rhythm: `reply_tempo` EMA per room (O(1) per turn, outlier-capped so an
  ...
  (`scripts/tk1/seed_behavior_rules.py`, `--apply` is the Captain's hand). +15 tests
...
- **GO-LIVE, the same day** (the two steps the roadmap was still holding, both now done): the frozen
  notice is POSTED in `#privacy` (`scripts/tk1/post_consent_notice.py`, dry-run by default — it opens its
