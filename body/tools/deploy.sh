...
#
#     body/tools/deploy.sh [--dry-run] [--tag <name>] [--when-quiet [max-wait-s]] [--no-rollback] [--skip-gate]
#
...
The body's address comes from, in order:
  ...
  2. body/tools/body.conf (git-ignored), which may set:
...
#    SILENT (runbook §2.4). So: install the newest body/tools/requirements-lock-*.txt first, then
...
install_deps() {
    local lock
    lock="$(remote "cd ${BODY_REPO} && ls -1 body/tools/requirements-lock-*.txt 2>/dev/null | sort | tail -1")"
    [ -n "${lock}" ] || die "no body/tools/requirements-lock-*.txt on the body — refusing to resolve
...
DEPS_TOUCHED=0
if [ "${DRY_RUN}" -eq 1 ]; then
    remote "cd ${BODY_REPO} && git diff --name-only <OLD> <NEW> -- tokeniko-tk1/pyproject.toml tokeniko-tk1/requirements.txt 'body/tools/requirements-lock-*.txt'"
    remote "cd ${BODY_REPO} && ls -1 body/tools/requirements-lock-*.txt | sort | tail -1"
    ...
    DEP_DIFF="$(remote "cd ${BODY_REPO} && git diff --name-only ${OLD_SHA} ${NEW_SHA} -- tokeniko-tk1/pyproject.toml tokeniko-tk1/requirements.txt 'body/tools/requirements-lock-*.txt'")"
    if [ -n "${DEP_DIFF}" ]; then
        printf '%s\n' "${DEP_DIFF}" | sed 's/^/        /'
...
if [ "${DRY_RUN}" -eq 1 ]; then
    remote "cd ${BODY_REPO} && .venv/bin/python body/tools/preflight_models.py"
else
    if ! remote "cd ${BODY_REPO} && .venv/bin/python body/tools/preflight_models.py"; then
