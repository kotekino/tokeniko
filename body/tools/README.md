# `body/tools/` — the body's furniture
...
cp body/tools/online.tokeniko.api.plist    ~/Library/LaunchAgents/
cp body/tools/online.tokeniko.brain.plist  ~/Library/LaunchAgents/
cp body/tools/online.tokeniko.senses.plist ~/Library/LaunchAgents/
...
for s in api brain senses; do
  cp body/tools/online.tokeniko.$s.plist ~/Library/LaunchAgents/
...
./.venv/bin/python body/tools/verify_transplant.py --out body/tools/baseline-<date>.json

# on the mini, after the rsync and `docker compose up -d` — diff against it
./.venv/bin/python body/tools/verify_transplant.py --compare body/tools/baseline-<date>.json
...
body/tools/deploy.sh --dry-run     # read every remote command before trusting it
body/tools/deploy.sh               # for real
```
