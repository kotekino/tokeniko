...
`E3.8` *(sub-items: E3.8.1 · E3.8.2)* **⚑ THE FRAME/KNOWLEDGE AUDIT — the epic's closing act** *(the Captain, 2026-09-16, and the first
   ...
   **The candidates already on the list, named as they were built rather than remembered at the end:**
   ...
   | ~~`UD_DEP_TO_ROLE`~~ **MOVED 09-24 — the sets are rows (`db/0034`), the ranking DELETED: benched, it never decided (`202609240900_…`). And the Captain ruled frame is not «what goes in code»** (`closed.py`) | *as flagged 09-22:* WHICH roles a relation admits is shape-reading — frame. The ORDER, which is BEST-FIRST, is a claim about which reading is commoner in English — knowledge, revisable by counting. They are welded because the dict value is one tuple |
