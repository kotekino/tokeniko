# docs/dictionary/origin-prototype — the dictionary instrument (né dictionary-review)
...
python docs/dictionary/origin-prototype/tk2_subset.py                    # report only
python docs/dictionary/origin-prototype/tk2_subset.py --apply --name v1  # store it

# 2. the matrix — rows = dimensions, POS-split, every cell tagged with the relation that made it
python docs/dictionary/origin-prototype/tk2_matrix.py --subset v1 --run r1 --note "all relations on" --apply
python docs/dictionary/origin-prototype/tk2_matrix.py --subset v1 --run r2-nogloss --weight gloss_overlap=0 --apply

# 3. the bar — declared in tk2_config.PAIRS BEFORE any matrix existed
python docs/dictionary/origin-prototype/tk2_probe.py --run r1 --compare r2-nogloss

# 4. the two-matrix base — the prototype that finding 4 forced (stage 2b)
python docs/dictionary/origin-prototype/tk2_build2.py --apply

# 5. interrogate it — the draft of the tk2 API surface
python docs/dictionary/origin-prototype/tk2.py bar
python docs/dictionary/origin-prototype/tk2.py near sleep.v
python docs/dictionary/origin-prototype/tk2.py rel eat.v food.n
python docs/dictionary/origin-prototype/tk2.py opposite leave
python docs/dictionary/origin-prototype/tk2.py effect eat

# 6. definitional curation — PROPOSALS ONLY, the Captain approves by eye (stage 4)
python docs/dictionary/origin-prototype/tk2_curate.py propose
python docs/dictionary/origin-prototype/tk2_curate.py simulate
```
