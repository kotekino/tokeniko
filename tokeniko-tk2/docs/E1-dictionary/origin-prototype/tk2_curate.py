...
speaks — DIRECTED, `bed.n -> sleep.v`, because that is the direction the statement runs in.
    ...
    python docs/E1-dictionary/origin-prototype/tk2_curate.py propose                       # the four bar misses
    python docs/E1-dictionary/origin-prototype/tk2_curate.py propose --pairs eat.v:food.n,bed.n:sleep.v
    python docs/E1-dictionary/origin-prototype/tk2_curate.py propose --word sleep.v        # one key against every dimension
    python docs/E1-dictionary/origin-prototype/tk2_curate.py simulate                      # would it close the gaps?
    python docs/E1-dictionary/origin-prototype/tk2_curate.py approve <id> ... --i-am-the-captain
...
def load_keys(db, base: str) -> list[str]:
    ...
    if not man:
        raise SystemExit(f"no two-matrix base '{base}' in {CFG.TK2_DB}.base2_manifest — "
                         f"run: python docs/E1-dictionary/origin-prototype/tk2_build2.py --apply")
