...
4. **One round-trip fence, both directions** — meaning proven preserved or the original ships (raw input in, the DECOMPILED sentence verbatim out — renamed 2026-09-19, parser-compiler req 9).
5. **Optional by design** — rag off, he still works: parser tolerance in, the decompiled sentence out; graceful degradation, never a hard dependency. *That is what makes the decompiler's output have to be honest English rather than a debug readout: with rag off it is what ships.*
