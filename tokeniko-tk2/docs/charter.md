# tokeniko 2 — THE CHARTER *(the phase-1 record)*

> *Was `docs/README.md` until 2026-09-17, and renamed because that is not what it is: the README's
> job is to describe the project and is done in `../README.md`. **This is the charter** — the two
> limits tk2 exists to kill, the continuity contract, the governing decisions, the inheritance and
> open-questions ledgers, the success criteria, and the requirements method. It is reference
> material, not a status document, and it is the record of the conception phase that closed
> 2026-08-13.*
...
`docs/` holds the conception work for the next architecture. Everything here is **reference
...
| 6 | Scope and binders | both | **ANSWERED 2026-09-11 (E2 t2)** — **row order = scope order**, tested against an 8-case bench against four candidates. The prefix carries FIVE scope-bearing elements (quantifier · negation · modality · attitude · domain), so row order is never free for anything else — which is also why `CONV` cannot be `IMPLY` with the rows swapped. tkzip reqs 35, 36, 42 |
...
| 8 | Nesting via named rows | — | **DONE 2026-09-11 (E2 t2)** — a zip is a FLAT LIST, nesting is naming. A box may hold a row name, valued by that row's derived point. Attitudes beyond depth 1 take a prefix position (de re / de dicto), with the flat POV field kept as shorthand. tkzip reqs 33, 34, 45 |
| 9 | Storage width | — | **ANSWERED 2026-09-12 (E2 t5)** — sparse, and the proposal is now measured: PropBank's 112,917 predicate instances give mean 2.59 roles of 18 slots, **~80% of every row EMPTY**. The KEY is the truth and the VECTOR an epoch-stamped cache (dictionary req 13), so a dictionary rebuild invalidates only the cache — **E9's translation night is for a SCHEMA change, never for a new base**. tkzip reqs 61–63 |
...
| 13 | Role inventory completeness | both | **CLOSED 2026-09-11 (E2 t1)** — 17 boxes + the predicate, cut from VerbNet (29 roles / 429 classes), PropBank (112,917 annotated sentences) and FrameNet (1,221 frames), cross-checked against the Captain's own first draft. `comitative` was found in neither computational inventory and in the draft. See `E2-tkzip/202609111051_notes.md` |
| 14 | **The kinds of name — whose closed set?** OntoNotes' 18 NER labels (what the models actually emit) or our own inventory with a map from theirs to ours | Cap | **ANSWERED 2026-09-16** — ours, **and as ROWS rather than code**: *«with our inventory as db (not code!) we can modify it keeping the logic: everything is KB. If we rely on a third party we lose this ability.»* That is the deepest of the reasons and it goes further than the four engineering ones (see parser-compiler req 17): a set that feels like law and is revisable is curation, so the kinds AND the label map are both rows and only the reader is frame. **Prominence is a scalar, not a kind** — «public personality» is a person who is prominent, and it is what the micro-nn ranks |
| 15 | **E3b's micro-nn and E5's lib — which comes forward?** E3b declares the ecosystem's first instance, and E5 is the framework it declares against | Cap | **ANSWERED 2026-09-16** — *«bring E5 forward because it is propaedeutic to closing the E3 name hole. So E5 remains where it is, but the machinery (micro-nn abstraction) should go in E3b»*. **The epic stays; the ABSTRACTION moves** (E3b task 5a; micro-nn reqs 1–4, 7, 8). Not a scheduling call: a framework generalized before it has met a real case is the failure mode «one abstraction, many instances» is most exposed to, and its first case now arrives with it. My own lean — hold the net and abstain — was the cheaper answer and the weaker one |
| 16 | **Identity binding across channels.** Individual uids are `name@channel:talker_uid`, so a new channel mints a NEW individual unless deliberately bound | Cap | **OPEN, AND PINNED 2026-09-16** — the Captain agreed the decision point is **E3b task 6, before the first uid is minted**. Flagged in `ideas.md` before any of this: *«decide the binding before the first line is typed, not after»*, and E3b is when names start minting |
...
- **Documents live here.** Anything referenced by the blueprint gets a file in `docs/`.
...
```
docs/<component>/YYYYMMDDHHMM_notes.md   the dialogue that produced the thinking, dated, kept
docs/<component>/requirements.md          SUPER synthetic — what the design must respect
```
